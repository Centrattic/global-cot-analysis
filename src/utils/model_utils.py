            ...
            if hasattr(model, 'transformer') and hasattr(
                ...
                if layer_idx < len(model.transformer.h):
                    model.transformer.h[layer_idx].register_forward_hook(
                        hook_fn(model.transformer.h[layer_idx], None, None,
                                layer_idx))
...
MODEL_CONFIGS = {
    ...
    ModelConfig(model_name="anthropic/claude-opus-4-20250514",
                thought_tokens=["<think>", "</think>"],
                response_tokens=["</think>", "<｜end▁of▁sentence｜>"],
                activation_hook_config=None,
                provider="anthropic"),
    "claude-sonnet-4.5":
    ModelConfig(model_name="anthropic/claude-sonnet-4.5",
                thought_tokens=["<think>", "</think>"],
                response_tokens=["</think>", "<｜end▁of▁sentence｜>"],
                activation_hook_config=None,
                provider="anthropic"),
    "claude-sonnet-3.7":
    ModelConfig(model_name="anthropic/claude-3.7-sonnet:thinking",
                thought_tokens=["<think>", "</think>"],
                response_tokens=["</think>", "<｜end▁of▁sentence｜>"],
                activation_hook_config=None,
                provider="anthropic"),
...
def get_activation_hook(model_name: str) -> Optional[ActivationHook]:
    ...
    hook_kwargs = {
        k: v
        for k, v in config.activation_hook_config.items() if k != "hook_class"
    }
