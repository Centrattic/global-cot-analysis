...
def find_matching_clusters(prefix_chunks: List[str],
...
def find_flowchart_path(prompt: str, config_name: str, f_config_name: str = None) -> Optional[Path]:
    ...
    if f_config_name is None:
        ...
        for flowchart_file in flowchart_dir.glob("*.json"):
            if (flowchart_file.name.startswith(f"config-{config_name}-")
...
def get_actual_distribution_for_prefix(
        prefix_name: str,
        prefix_correctness_data: Dict[str, Any]) -> Dict[str, float]:
...
def save_comparison_csv(comparison_results: Dict, output_path: str):
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        writer.writerow([
            'prefix_name', 'response', 'predicted_percentage',
...
def run_prefix_prediction_comparison(
    ...
    print(
        f"Processing {len(prefixes_to_process)} prefixes from config: {prefixes_to_process}"
    )
