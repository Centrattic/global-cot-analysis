...
class PropertyRunner:
    ...
    def run_properties_from_config(self, config_name: str):
        ...
        for model in models:
            self._process_model_rollouts(prompt_index, model,
                                         property_checker_names)
            self._process_model_resamples(prompt_index, model,
                                          property_checker_names)
    ...
    def _process_model_rollouts(self, prompt_index: str, model: str,
        ...
        json_files = [
            f for f in os.listdir(rollout_dir) if f.endswith('.json')
        ]
    ...
    def _process_model_resamples(self, prompt_index: str, model: str,
        ...
        prefix_dirs = [
            d for d in os.listdir(resamples_dir)
            if os.path.isdir(os.path.join(resamples_dir, d))
        ]
        ...
        for prefix_dir in prefix_dirs:
            ...
            self._process_prefix_resamples(prefix_path, prompt_index,
                                           property_checker_names, prefix_dir)

    def _process_prefix_resamples(self, prefix_path: str, prompt_index: str,
                                  property_checker_names: List[str],
                                  prefix_name: str):
        ...
        json_files = [
            f for f in os.listdir(prefix_path) if f.endswith('.json')
        ]
        ...
        try:
            ...
            for checker_name in property_checker_names:
                if checker_name in self.property_checkers:
                    ...
                    print(
                        f"  Warning: Property checker {checker_name} not found"
                    )
    ...
    def _process_flowcharts(self, prompt_index: str, models: List[str],
                            property_checker_names: List[str]):
    ...
    def _process_single_flowchart(self, flowchart_path: str, prompt_index: str,
        ...
        try:
            ...
            if not flowchart_data or "responses" not in flowchart_data:
                print(
                    f"Skipping flowchart without responses: {flowchart_path}")
                return
    ...
    def _update_flowchart_response_properties(
            self, response_data: Dict[str, Any], prompt_index: str,
            property_checker_names: List[str]) -> bool:
        ...
        try:
            ...
            if seed is None:
                print(
                    f"    Warning: seed field is None for response, skipping")
                return False
            ...
            if resampled is False:
                ...
                if not source_file_path:
                    print(
                        f"    Warning: Could not find rollout file for seed {seed}"
                    )
                    return False
            else:
                ...
                source_file_path = None
                for model in self.config["models"]:
                    potential_path = f"prompts/{prompt_index}/{model}/resamples/{prefix}/{seed}.json"
                    if os.path.exists(potential_path):
                        source_file_path = potential_path
                        break
            ...
            if not source_data:
                print(
                    f"    Warning: Could not load source file: {source_file_path}"
                )
                return False
            ...
            for checker_name in property_checker_names:
                if checker_name in source_data:
                    ...
                    if old_value != new_value:
                        ...
                        updated = True
                        print(
                            f"    Updated {checker_name}: {old_value} -> {new_value}"
                        )
    ...
    def _process_files_batch_parallel(self, file_paths: List[str],
                                      prompt_index: str,
                                      property_checker_names: List[str]):
    ...
    def _process_algorithm_parallel(self, file_paths: List[str],
                                    prompt_index: str) -> Dict[str, str]:
        ...
        for i, file_path in enumerate(file_paths):
            try:
                ...
                if response_data and response_data.get(
                        "cot_content") and response_data.get(
                            "response_content"):
                    response_data_list.append(response_data)
        ...
        print(
            f"    Processing {len(valid_responses)} files in parallel for algorithm detection..."
        )
