...
class PropertyCheckerMultiAlgorithm(PropertyCheckerMulti):
    ...
    def _get_cues_for_algorithm(self, cues_map: Dict[str, List[str]],
                                alg: str) -> List[str]:
        """Get the cues list for a given algorithm identifier."""
        return cues_map.get(alg, [])

    def _heuristic_keywords_output(self, sentences: List[str],
                                   cues_map: Dict[str, List[str]]) -> str:
        ...
        for idx, s in enumerate(sentences, start=1):
            ...
            for alg in algorithm_labels:
                ...
                if has_cue:
                    positions = [
                        s.lower().find(p.lower()) for p in cues
    ...
    def get_value_for_node(self,
                           sentences: List[str],
                           prompt_index: str = None) -> List[List[str]]:
