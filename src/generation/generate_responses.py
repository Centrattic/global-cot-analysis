    ...
    def _generate_resamples_for_model(
        ...
        for prefix in prefixes:
            ...
            if not seeds_to_generate:
                ...
                if recompute:
                    print(
                        f"Recomputing {len(seeds_to_generate)} resamples for prefix {prefix}"
                    )
                else:
                    print(
                        f"Generating {len(seeds_to_generate)} new resamples for prefix {prefix}"
                    )
            ...
            def generate_single_resample(seed):
                ...
                reasoning_text = response_data.get("reasoning") or "" # reasoning is None when empty
    ...
    def _call_api(
        ...
        try:
            ...
            if force_cot and model == "gpt-oss-20b":
                ...
                raw_prompt = f"<|start|>user<|message|>{prompt}<|end|>\n{thought_token}{prefix_text}"
