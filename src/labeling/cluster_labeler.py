...
def build_prompt(prompt_text: str, cluster_texts: List[str]) -> str:
    ...
    cluster_block = "\n".join(f"- {t}"
                              for t in cluster_texts[:20])  # cap for safety
    return (
...
def label_flowchart(flowchart: Dict[str, Any],
                    prompt_text: str,
                    max_workers: int | None = None) -> Dict[str, Any]:
    # flowchart["nodes"] is a list of {cluster_key: node_data}
    nodes = [
        n for n in flowchart.get("nodes", []) if isinstance(n, dict) and n
    ]
    ...
    def run_prompt(
        ...
        ck, _, pr, texts = item
        primary_model = os.environ.get('CLUSTER_LABEL_PRIMARY_MODEL',
                                       'openai/gpt-4o-mini')
        secondary_model = os.environ.get('CLUSTER_LABEL_SECOND_MODEL',
        ...
        if s1 == 'A':
            ...
            if mentions >= 2:
                ...
                if s2 == 'A':
                    ...
                    extract_prompt = ("S contains a number. Extract it.\n"
                                      f"S: {representative}\n"
                                      "Only output the number from S.")
                    out = call_llm(extract_prompt,
...
def label_flowchart_file(flowchart_path: str,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Add cluster labels to an existing flowchart JSON")
    ...
    label_flowchart_file(args.flowchart,
                         prompts_json_path=args.prompts,
